...
consisting of a disk suspended by three wires. Objects are placed in the middle of the disk, and the disk is given a small initial perturbation resulting in small oscillations around the mass centre.
...
\section{Results}
Experimental characteristics of the Trifilar were measured prior to undertaking the experiment. Measurements included disk weight $m_d$; disk radius $r_d$; filament length $L$; and the radius from centre of disk to middle set of holes $R$. These measurements are presented in Table 1 below. To better understand where the masses were mounted on the disk, the Trifilar disk geometries are shown in Figure 3.
...
\begin{figure}[h]
	...
	\begin{minipage}{0.45\textwidth}
		...
		\begin{tabular}{lrc}
			...
			\midrule
			Disk Mass $(m_d)$ & 0.20 & $\si{\kilogram}$\\
			Disk Radius $(r_d)$ & 0.14 & $\si{\meter}$ \\
			Filament Length $(L)$ & 0.45 & $\si{\meter}$ \\
			Distance to Middle Holes $(R)$ & 0.09 & $\si{\meter}$\\
			\bottomrule
		\end{tabular}
	\end{minipage}
	\hspace{1cm}
